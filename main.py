...
def verificar_selecao():
    if not checkbox_123_var.get() and not checkbox_abc_var.get() and not checkbox_data_var.get():
        messagebox.showerror("Erro", "Selecione um padrão")
        return False
    return True
...
def mostrar_tela_carregamento():
    ...
    tela_carregamento.title("Carregando...")
    ...
    label_carregando = ctk.CTkLabel(tela_carregamento, text="Renomeando arquivos...")
    ...
    label_progresso = ctk.CTkLabel(tela_carregamento, text="0 arquivos renomeados.")
...
def renomear_sequencial(pasta, nome_base, arquivos, barra_progresso, label_progresso, total_arquivos):
    if nome_base == "":
        for i, arquivo in enumerate(arquivos, 1):
            ...
            label_progresso.configure(text=f"{i} arquivos renomeados.")
...
def renomear_alfabetico(pasta, nome_base, arquivos, barra_progresso, label_progresso, total_arquivos):
    if nome_base == "":
        for i, arquivo in enumerate(arquivos):
            ...
            label_progresso.configure(text=f"{i + 1} arquivos renomeados.")
            label_progresso.update()
    else:
        for i, arquivo in enumerate(arquivos):
            sufixo = indice_para_letras(i)
...
def renomear_arquivos():
    ...
    messagebox.showinfo("Sucesso!", f"{total_arquivos} arquivos renomeados!")
...
def abrir_link():
    url = "https://www.bugzinho.com/renamethis-br"
    webbrowser.open(url)
...
label = ctk.CTkLabel(root, text="Selecione a pasta:", bg_color='#151515')
...
botao_buscar = ctk.CTkButton(root, text="Buscar", command=selecionar_pasta,
                             fg_color="#483d8b", hover_color="#2c226a",
                             bg_color='#151515')
botao_buscar.place(x=430, y=20)  # Define a posição (x=460, y=20)

# Label e entrada de renomear
label_renomear = ctk.CTkLabel(root, text="Renomear para:", bg_color='#151515')
...
# Botão "Renomear"
botao_renomear = ctk.CTkButton(root, text="Renomear",
